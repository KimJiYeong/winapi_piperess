...
BOOL _find(int posY, int posX, HWND hWnd) { //좌표를 받아온다.
	...
	if (Cycle == TRUE) {//사이클이 트루일때만
		if (output[posY][posX].save_sel == 2)
		{
			if (look == 0) {
				posX -= 1;
			}
			...
			if (output[posY][posX].save_sel == -1)	//예외
			{
				Cycle = FALSE;
			}
			else if ((output[posY][posX].save_sel == 3 || output[posY][posX].save_sel == 1 || output[posY][posX].save_sel == 4) && look == 0)//ㄴ 모양
			{
				Count++;
				switch (output[posY][posX].save_sel)
				{
				case 1:
					look = 0;
					break;
			...
			else if ((output[posY][posX].save_sel == 5 || output[posY][posX].save_sel == 0 || output[posY][posX].save_sel == 4) && look == 1)//ㄴ 모양
			{
				Count++;
				switch (output[posY][posX].save_sel)
				{
		...
		else if (output[posY][posX].save_sel == 3) {
			if (look == 2) {
				posX += 1;
			}
			else {
			...
			else if ((output[posY][posX].save_sel == 5 || output[posY][posX].save_sel == 1 || output[posY][posX].save_sel == 2) && look == 2)//ㄴ 모양
			{
				Count++;
				switch (output[posY][posX].save_sel)
				{
		...
		else if (output[posY][posX].save_sel == 4) {
			if (look == 2) {
				posX += 1;
			}
			else {
			...
			else if ((output[posY][posX].save_sel == 0 || output[posY][posX].save_sel == 2 || output[posY][posX].save_sel == 3) && look == 3)//ㄴ 모양
			{
				Count++;
				switch (output[posY][posX].save_sel)
				{
		...
		else if (output[posY][posX].save_sel == 5) {
			if (look == 0) {
				posX -= 1;
			}
			else {
		...
		else if (output[posY][posX].save_sel == 0) {
			if (look == 1) {
				posY -= 1;
			}
			else {
		...
		else if (output[posY][posX].save_sel == 1) {
			if (look == 0) {
				posX -= 1;
			}
			else {
		...
		else if (output[posY][posX].save_sel == -1) { // save_sel이 -1 이면
			Cycle = FALSE;
			Sele = FALSE;
		}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) //CALLBACK(윈도우 핸들, 처리해야 할 메시지의 값,입력장치의 값1,2)
{
	...
	static BOOL ui_title;//타이틀 BOOL 
	static HBITMAP bt_title; //타이틀 함수
	
	static HBITMAP bt_ui_game_over, bt_ui_game_pause; //타이틀 함수
	...
	case WM_CREATE:
		...
		bt_title = LoadBitmap(g_hinst, MAKEINTRESOURCE(303));
		bt_ui_game_over = LoadBitmap(g_hinst, MAKEINTRESOURCE(301));
		bt_ui_game_pause = LoadBitmap(g_hinst, MAKEINTRESOURCE(302));

		//블록 1 , 2 -> 일자블록
		//큐에다가 집어넣어주자!
		for (int i = 0; i < 10; i++) {
			que_save = rand() % 5;
			in_sel(que_save); //큐에다가 초기 값을 10번 집어넣는다.
		}
		//보드 판 크기 정의
	...
	case WM_CHAR:
		...
		if (wParam == 'k' || wParam == 'K') {
			if (ui_title) {
				...
				//PlaySound(MAKEINTRESOURCE(124), g_hinst, SND_RESOURCE | SND_LOOP | SND_ASYNC);
				PlaySound(MAKEINTRESOURCE(124), g_hinst, SND_RESOURCE | SND_ASYNC | SND_LOOP);
		...
		//우선 중 사이즈만 해주자!
			...
			for (int j = 0; j < Board_sizeY; j++) {
				...
				for (int i = 0; i < now_size * 5; i++) {
					...
					output[j][i].save_sel = -1; //이미지 저장
					output[j][i].Draw_true = FALSE;
				}
			}
			//설정 모두 저장 후에 다시 시작
			SetTimer(hWnd, 1, Set_time, NULL);
		 ...
		 //일시정지
			...
			if (stop % 2 == 1) {
				...
				PlaySound(MAKEINTRESOURCE(125), g_hinst, SND_RESOURCE | SND_ASYNC | SND_PURGE);
				KillTimer(hWnd, 1);
			}
			else {
				ui_bool_game_pause = FALSE;
	...
	case WM_TIMER:
		...
		case 1:
			...
			for (int i = 0; i < now_size * 5; i++) { //x 좌표
				//배경 이미지를 설정해준다.
				if (fake[i].save_y == (All_sizeY / Board_sizeY)) {
					PlaySound(MAKEINTRESOURCE(125), g_hinst, SND_RESOURCE | SND_ASYNC );
				...
				if (fake[i].save_x == Block.move_x && fake[i].save_y == Block.move_y) {
					...
					output[Block.down_count][i].save_sel = Block.select; //이미지 저장
																		//--위치를 출력
					output[Block.down_count][i].save_x = fake[i].save_x; //너는 정상이고
					output[Block.down_count][i].save_y = fake[i].save_y; //이게 문제인것 같은데...
					output[Block.down_count][i].Draw_true = TRUE;
					...
					//디렉션을 정해보자
					switch (output[Block.down_count][i].save_sel)
					{
					case 1:
						look = 0;
						break;
		...
		break;
	case 2:
		if (All_time_count % 300 == 0) { //시간이 지나면 점점 속도가 빨라진다. //30초 마다 한번씩
			if (Set_time > -20) {
				Set_time -= 20;
			}
			KillTimer(hWnd, 1);
	...
	case 3:
		...
		//if (Ani_bool) {
			if (Ani_save == Ani_return_count) {
				...
				for (int i = now_size * 10; i >= 0; --i) { //처음부터 내려오는거
					for (int j = now_size * 5; j >= 0; --j) { //처음부터 내려오는거
						while (i < now_size * 10 - 1 && output[i][j].Draw_true == TRUE && output[i + 1][j].Draw_true == FALSE) {
							output[i + 1][j].Draw_true = TRUE;
							output[i][j].Draw_true = FALSE;

							output[i + 1][j].save_sel = output[i][j].save_sel;
							output[i][j].save_sel = -1;
							fake[j].save_y = output[i][j].save_y;
							i++;
						}
						Sele = _find(i, j, hWnd);//
						//배경 원래 색으로 돌아오기
						if (output[now_size * BG_UI_fake_count][j].save_sel == -1) {
							if (BG_UI_count > 1) {
								BG_UI_count--;
							}
							if (BG_UI_fake_count < 7) {
								BG_UI_fake_count += 2;
							}
						}
					}
				...
				if (Ani_timer% 4 == 0) {
					...
					output[Ani[Ani_save].count_y][Ani[Ani_save].count_x].Draw_true = FALSE;
					output[Ani[Ani_save].count_y][Ani[Ani_save].count_x].save_sel = -1;
					fake[Ani[Ani_save].count_x].save_y += (All_sizeY / Board_sizeY);
					Ani_save++;
					PlaySound(MAKEINTRESOURCE(125), g_hinst, SND_RESOURCE | SND_ASYNC );
				...
				//폭탄 애니메이션
				Ani_bomb_count += 1;
				Ani_bomb_count = Ani_bomb_count % 3;
	...
	case WM_PAINT:
		...
		for (int i = 0; i < now_size * 10; i++) {
			for (int j = 0; j < now_size * 5; j++) {
				if (output[i][j].Draw_true) {
					//배경 색 변경
					if (output[now_size * BG_UI_fake_count][j].save_sel != -1) {
						if (BG_UI_count < 6) {
							BG_UI_count++;
						}
						if (BG_UI_fake_count > 0) {
							BG_UI_fake_count -= 2;
						}
					}
										//휴우 일단 완성
					SelectObject(boardDC, hBlock_image[output[i][j].save_sel]);
					TransparentBlt(memdc, output[i][j].save_x, output[i][j].save_y, (All_sizeX / Board_sizeX), (All_sizeY / Board_sizeY), boardDC, 0, 0, 50, 50, RGB(255, 255, 255));
				}
				//애니메이션 출력
				if (Ani_bool) {
					...
					TransparentBlt(memdc,
						output[Ani[Ani_save].count_y][Ani[Ani_save].count_x].save_x,
						output[Ani[Ani_save].count_y][Ani[Ani_save].count_x].save_y,
					...
					if (Ani_save != 0 ) {
						...
						aniDC = CreateCompatibleDC(memdc);
						SelectObject(aniDC, hAni_Bomb[Ani_bomb_count]);
						TransparentBlt(memdc,
							output[Ani[Ani_save - 1].count_y][Ani[Ani_save - 1].count_x].save_x,
							output[Ani[Ani_save - 1].count_y][Ani[Ani_save - 1].count_x].save_y,
							(All_sizeX / Board_sizeX), (All_sizeY / Board_sizeY), aniDC, 0, 0, 150, 150, RGB(0, 0, 255));
						DeleteDC(aniDC);
		...
		if (ui_bool_game_pause) {//게임 일시정지 화면
			uiDC = CreateCompatibleDC(memdc);
			SelectObject(uiDC, bt_ui_game_pause);
			//BitBlt(memdc, 0, 0, All_sizeX, All_sizeY, uiDC, 0, 0, SRCPAINT);
			BLENDFUNCTION t;
			t.BlendOp = AC_SRC_OVER;
			t.BlendFlags = 0;
			t.SourceConstantAlpha = 200;
			t.AlphaFormat = 0;
			AlphaBlend(memdc, 0, 0, All_sizeX, All_sizeY, uiDC, 0, 0, All_sizeX, All_sizeY, t);
			DeleteDC(uiDC);
		}
		
		if (ui_bool_game_over) {//게임 오버 화면
			uiDC = CreateCompatibleDC(memdc);
			SelectObject(uiDC, bt_ui_game_over);
			BLENDFUNCTION t;
			t.BlendOp = AC_SRC_OVER;
			t.BlendFlags = 0;
			t.SourceConstantAlpha = 200;
		...
		if (ui_title) {
			uiDC = CreateCompatibleDC(memdc);
			SelectObject(uiDC, bt_title);
			StretchBlt(memdc, 0, 0, 900, 1100, uiDC, 0, 0, 1100,900, SRCCOPY);
			DeleteDC(uiDC);
		}
	...
	case WM_LBUTTONDOWN:
		...
		for (int i = 0; i < 6; i++) {
			if (PtInRect(&ui[i].re, end_mouse)) { //&rect , Point
				//stop은 예외 처리 해준다.
